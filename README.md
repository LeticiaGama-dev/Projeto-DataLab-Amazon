...
Este projeto foi desenvolvido no cenário da "Datalab", uma consultoria fictícia de análises de dados que incentiva seus analistas a explorarem datasets alinhados aos seus interesses para demonstrar seu potencial analítico.
...
* **Mito: Descontos geram melhores notas.**  
  **Refutado:** Não há correlação estatística significativa entre `discount_percentage` e `rating`.  
...
* **Mito: O número de avaliações afeta a nota média.**  
  **Refutado:** Não há correlação entre `rating_count` e `rating`.  

---

## Recomendações Estratégicas (Para a "Datalab")

1. **Foco Imediato na Qualidade do Cadastro (Quality Control):**  
   A prioridade de investimento deve ser em auditoria de marketplace para **eliminar anúncios sem imagem**, especialmente em categorias de alto valor como "Electronics", onde produtos caríssimos foram encontrados sem fotos.  
2. **Repensar a Estratégia de Descontos:**  
   A empresa não deve "gastar" margem de lucro com descontos na expectativa de melhorar a satisfação ou a nota média, pois os dados mostram que esses fatores não estão correlacionados.

---
## Visualizações e Análises Chave

### 1. Dashboard de Insights e Testes Estatísticos
* **Descrição:** Dashboard que resume os principais testes estatísticos e visualizações exploratórias (como dispersão e contagem) realizados no projeto. Apresenta o contexto do dataset (total de produtos, usuários) e evidencia análises sobre a relação entre variáveis como **desconto, preço e categoria versus a média de avaliação** (`rating`).

![Dashboard - Insights e Testes Estatísticos](dashboards_screenshots/dash1-proj%2004.jpg)

---

### 2. Resultados e Conclusões sobre Risco Relativo (RR)
* **Descrição:** Sumário visual dos achados principais do projeto. Utiliza o conceito de **Risco Relativo (RR)** para testar a influência de diferentes variáveis (preço, imagem e categoria) na probabilidade de um produto receber avaliações. O principal insight destacado é que a **presença de imagem** é uma condição crucial para receber avaliações.

![Resultados - Risco Relativo](dashboards_screenshots/dash2-proj%2004.jpg)

---

### 3. Recomendações Estratégicas e Constatações
* **Descrição:** Compilação das **constatações** do projeto e das **recomendações estratégicas** derivadas delas. As recomendações são divididas em pilares chave, como a **Prioritização do uso de Imagens**, a reorientação da estratégia de **Preço e Desconto**, o **Monitoramento da Qualidade da Avaliação** e a **Comunicação Interna** com vendedores.

![Recomendações Estratégicas](dashboards_screenshots/dash3-proj%2004.jpg)
...
* **Onde foi aplicada:** Para testar as hipóteses de correlação entre:  
...
## Processo Técnico (ETL e Feature Engineering)

* **Tratamento de Nulos e Duplicados:** Ajustes básicos para garantir consistência do dataset.
* **Feature Engineering:** Criação das colunas `categoria_principal` e `status_imagem` para segmentação e análise estatística.
* **União das Tabelas:** Combinação dos dados de produtos e reviews via LEFT JOIN, mantendo produtos sem avaliações.
...
O projeto foi desenvolvido no Google Colab utilizando Python e Pandas para ETL e análise.  
Para replicar a análise, os seguintes passos foram aplicados:
1. Carregar os datasets `amazon_product.csv` e `amazon_review.csv`.
2. Limpeza de dados e criação de colunas derivadas (`categoria_principal` e `status_imagem`).
3. Análise Exploratória (visualizações e métricas de resumo).
4. Testes estatísticos aplicados para validar ou refutar hipóteses.
